...
def make_categorical_only(X_train,X_val, y_train,y_val, catdex):
	...
	pgrid={'base_estimator': [DCT(max_leaf_nodes=4), DCT(max_leaf_nodes=8)],'n_estimators':[2,3,5,7,10]}
	X_train =X_train[:,catdex:]
	X_val = X_val[:,catdex:]
...
def main():	
	...
	for season in Seasons:
		...
		y_hat_n,ll_n = evaluate_numeric(numeric_model,X_train,X_test,y_train,y_test,num_dex, "./Models/Ada_num_"+str(season)+".pkl")
		y_hat_c, ll_c = evaluate_categorical(cat_model,X_train,X_test,y_train,y_test,num_dex, "./Models/Ada_cat_"+str(season)+".pkl")
		y_hat_f, ll_f = evaluate_full(full_model,X_train,X_test,y_train,y_test,num_dex, "./Models/Ada_full_"+str(season)+".pkl")
		write_text(ll_n, ll_c, ll_f, "../Data/Ada_results_"+str(season)+".txt",season)
		
		plot_roc(y_test,[y_hat_n,y_hat_c,y_hat_f],"AdaBoost ROC Curves " + str(season)+ " Season","../Figs/Ada_ROC_"+str(season)+".pdf")
